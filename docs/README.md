...
- 技術
    ...
    - 外部 SDK や API との連携実装
    - プッシュ通知や Firebase Analytics の実装
...
- ドキュメント駆動開発
  - ドキュメントを基にした開発プロセスにより、AI エージェントによる開発を実現し、開発の効率と品質を向上させることができると考えています
...
Web 版電子カルテと連携するモバイルアプリの新規開発を担当。要件定義から設計・開発まで、プロジェクトの上流工程から参画。リード業務として技術選定やチームマネジメントを行いながら、クライアントとの信頼関係を構築し、プロジェクト完遂に導く。
...
途中参加ながら既存コードからキャッチアップし、遅延していた実装を進めてスケジュール通りの納品を実現。また、オンボーディング資料の整備や Slack での質問対応を積極的に行い、チームの生産性向上に貢献。リードエンジニア不在時には代理として Android 側のタスク管理も担当。
...
大手スポーツジムチェインのモバイルアプリの保守・運用を担当。外部ベンダーからの引き継ぎ後、少ない保守工数の中で Firebase 移管作業、OS アップデートやポリシー変更に追従するための計画的な対応をクライアントに提案し、プロジェクトを円滑に進行。
...
約 10 万人の MAU を持つゴルフ動画配信サービスの開発・保守・運用を担当。2022 年 3 月に開発リーダーに就任し、システム設計や外部サービス連携のドキュメントを整備することで、保守運用の負担軽減とリリース時の不具合ゼロを実現。開発の属人化を防ぎ、チームの技術力向上と自律的な開発体制の構築に取り組む。
...
大手国内銀行と大手小売店が共同運営する金融サービスの Android ネイティブアプリ開発を担当。詳細設計担当者の退職に伴い、Android 開発を単独で担当。仕様変更や SDK の問題に対応しながら開発を進め、最終的に納品を完遂。
